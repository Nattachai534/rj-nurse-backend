...
# --- Helper: จัดรูปแบบข้อมูล (ตัดช่องว่างทิ้ง) ---
def format_db_row(row, title_field):
    ...
    field_map = {
        "description": "รายละเอียด", "objective": "วัตถุประสงค์", "agenda": "วาระ", "detail": "เนื้อหา",
        "date_start": "วันเริ่ม", "date_end": "สิ้นสุด", "date_announce": "ประกาศเมื่อ",
        ...
        "location": "สถานที่", "room": "ห้อง",
        "link_register": "สมัคร", "link_doc_application": "เอกสารสมัคร",
        "link_announce_written": "ผลข้อเขียน", "link_announce_interview": "ผลสัมภาษณ์",
        "link_announce_final": "ผลผู้มีสิทธิ์", "link_poster": "รูปโปสเตอร์", "link_website": "เว็บ",
        "link_zoom": "Zoom", "zoom_meeting_id": "ID", "zoom_passcode": "Pass",
        "responsible_unit": "หน่วยงาน", "unit_phone": "โทร", "contact_person": "ติดต่อ", "contact_phone": "มือถือ",
        "process_status": "สถานะ", "position_name": "ตำแหน่ง", "date_close": "ปิดรับ", "date_open": "เปิดรับ"
    }
...
def query_mysql(user_query, role='guest'):
    ...
    try:
        ...
        if not any([fetch_training, fetch_meeting, fetch_project, fetch_unit, fetch_job, fetch_news]):
            fetch_training = True; fetch_news = True; fetch_project = True # Default search

        # Helper: Smart Fetch
        def smart_fetch(table, title_col, where_clause, order_clause, limit=10):
            # 1. ค้นหาเจาะจง
            ...
            # 2. Fallback: ถ้าไม่เจอ ให้ดึงข้อมูลล่าสุด
            if not rows:
...
def generate_bot_response(user_query, role='guest', user_name=None):
    ...
    # 🌟 Prompt ปรับปรุงใหม่: เน้นความเป็นมนุษย์ + การถามกลับ 🌟
    prompt = f"""
    บทบาท: คุณคือ "น้อง Bot RJ Nurse" ผู้ช่วยภารกิจด้านการพยาบาล รพ.ราชวิถี
    ...
    คำสั่งในการตอบ:
    1. **ห้ามมั่วข้อมูล:** ห้ามสร้างเบอร์โทร, ลิงก์ หรือวันที่ขึ้นมาเองเด็ดขาด ถ้าในข้อมูลอ้างอิงไม่มี ให้บอกว่า "ติดต่อหน่วยงานต้นสังกัด" หรือ "ยังไม่มีประกาศ"
    2. **กรณีข้อมูลเยอะ/ไม่เจาะจง:** ถ้าผู้ใช้ถามกว้างๆ (เช่น "ขอผลสอบ", "มีอบรมอะไรบ้าง") และในข้อมูลอ้างอิงมีหลายรายการ -> ให้สรุปรายชื่อสั้นๆ แล้ว **ถามผู้ใช้กลับ** ว่า "สนใจรายละเอียดของรายการไหนเป็นพิเศษไหมคะ?"
    3. **กรณีข้อมูลเจาะจง:** ถ้าผู้ใช้ถามชื่อโครงการชัดเจน หรือในข้อมูลมีแค่รายการเดียว -> ให้แสดงรายละเอียดทั้งหมดที่มี (วัน, เวลา, สถานที่, ลิงก์ต่างๆ, Zoom) ให้ครบถ้วนสวยงาม
    4. **การแสดงผล Zoom:** ถ้ามี Meeting ID/Passcode ให้แสดงคู่กันเสมอ
    5. ใช้ภาษาที่สุภาพ เป็นกันเอง (ค่ะ/คะ) และเป็นมืออาชีพ
    """
...
@app.get("/")
def root(): return {"status": "RJ Nurse Backend V22.0 Running"}
