...
def format_db_row(row, title_field):
    ...
    for k, v in row.items():
        ...
        # ไม่ใส่ Bullet (*) ข้างหน้า แต่ส่งไปให้ AI จัดการ Emoji เอง
...
def query_mysql(user_query, role='guest'):
    ...
    try:
        ...
        if not any([fetch_training, fetch_meeting, fetch_project, fetch_unit, fetch_job, fetch_news]):
            fetch_training = True; fetch_news = True; fetch_project = True
...
def generate_bot_response(user_query, role='guest', user_name=None):
    ...
    # 🌟 Prompt ปรับปรุงใหม่ V23.0: เน้นความสวยงามตามที่ User ขอ 🌟
    prompt = f"""
    ...
    คำสั่งในการจัดรูปแบบ (Formatting Rules):
    1. **ห้ามใช้ Markdown Table** เด็ดขาด
    2. **ห้ามใช้เครื่องหมายดอกจัน (*) หรือขีด (-) เป็น Bullet** นำหน้าบรรทัด (เช่น * หัวข้อ ไม่เอา)
    3. **ให้ใช้อีโมจิ (Emoji) นำหน้าหัวข้อแทน** เช่น:
       📢 **หน่วยงานจัด:** ...
       🗓️ **วันที่จัด:** ...
       📍 **สถานที่:** ...
       📋 **สถานะ:** ...
       🔗 **Link:** ...
    4. **ตัวหัวข้อ:** ให้ทำเป็น **ตัวหนา** (เช่น **วันที่:**)
    5. **ถ้าข้อมูลมี Zoom Meeting ID/Passcode:** ให้แสดงรวมไว้บรรทัดเดียวกับ Zoom Link หรือบรรทัดถัดมาให้ชัดเจน
    6. **ห้ามมั่วข้อมูล:** ถ้าไม่มีข้อมูลให้บอกว่า "ไม่พบข้อมูล" หรือ "ติดต่อหน่วยงาน"
    7. ใช้ภาษาไทยที่สุภาพและเป็นกันเอง
    """
...
@app.get("/")
def root(): return {"status": "RJ Nurse Backend V23.0 Running"}
